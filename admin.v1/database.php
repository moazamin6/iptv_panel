...
<head><title>phpMiniAdmin</title>
    ...
    <style type="text/css">
        * {
        ...
        table.sm th > div, table.sm td > div {
            max-height: 3.5em;
            overflow: hidden
        }

        table.sm th.lg, table.sm td.lg {
            max-width: inherit
        }

        table.sm th.lg > div, table.sm td.lg > div {
            max-height: inherit;
            overflow: inherit
        }
...
<form method="post" name="DF" id="DF" action="<?php eo($self) ?>" enctype="multipart/form-data">
    ...
    <div class="inv">
        ...
        <?php } ?>
        <?php if ($_SESSION['is_logged']) { ?> | <a href="?<?php eo($xurl) ?>&logoff=1"
    ...
    function print_screen()
    {
        ...
        <div class="dot">
            <div style="float:right;padding:0 15px"><label><input type="checkbox" name="is_sm" value="1" id="is_sm"
                                                                  onclick="smview()" <?php eo($is_sm ? 'checked' : '') ?>>
                    compact view</label></div>
...
function print_cfg()
{
    ...
    <center>
        ...
        <div class="frm">
            <label>
                <div class="l">DB user name:</div>
                <input type="text" name="v[user]" value="<?php eo($DB['user']) ?>"></label><br>
            <label>
                <div class="l">Password:</div>
                <input type="password" name="v[pwd]" value=""></label><br>
            <div style="text-align:right"><a href="#" class="ajax" onclick="cfg_toggle()">advanced settings</a></div>
            <div id="cfg-adv" style="display:none;">
                <label>
                    <div class="l">DB name:</div>
                    <input type="text" name="v[db]" value="<?php eo($DB['db']) ?>"></label><br>
                <label>
                    <div class="l">MySQL host:</div>
                    <input type="text" name="v[host]" value="<?php eo($DB['host']) ?>"></label> <label>port: <input
                            type="text" name="v[port]" value="<?php eo($DB['port']) ?>" size="4"></label><br>
                <label>
                    <div class="l">Charset:</div>
                    <select name="v[chset]">
                        <option value="">- default -</option><?php echo chset_select($DB['chset']) ?>
                    </select></label><br>
                <br><label for="rmb"><input type="checkbox" name="rmb" id="rmb" value="1" checked> Remember in cookies
                    for 30 days or until Logoff</label>
            </div>
            <center>
